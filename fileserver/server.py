...
# print error and die if a file directory isn't provided
if len(sys.argv) != 2:
	print("Usage: {} <file_directory>".format(sys.argv[0]))
	sys.exit(1)
...
def get_price_from_request(request):
	id = int(requset.args.get('selection'))
	return files[id][1]

# machine-payable endpoint that returns seleted file if payment made
@app.route('/buy')
